...
public class ServiceiLibrary : IServiceiLibrary
{
    ...
    public SetupLotResult SetupLot(string lotNo, string mcNo, string opNo, string processName ,string layerNo)
    {
        ...
        try
        {
            ...
            if (!proResult.IsPass)
            {
                return new SetupLotResult(SetupLotResult.Status.NotPass,proResult.Cause, "LotNo:" + lotNo + " opNo:" + opNo, "", MethodBase.GetCurrentMethod().Name, log);
            }
            ...
            if (!permissionResult.IsPass)
                  return new SetupLotResult(SetupLotResult.Status.NotPass, permissionResult.ErrorNo + ":" + permissionResult.ErrorMessage, "LotNo:" + lotNo + " opNo:" + opNo, "", MethodBase.GetCurrentMethod().Name, log);

         
            MachineInfo machineInfo = c_ApcsProService.GetMachineInfo(mcNo, log, dateTimeInfo.Datetime);
            if (machineInfo == null)
                return new SetupLotResult(SetupLotResult.Status.NotPass, "ไม่พบ MCNo :" + mcNo + " ในระบบ", "LotNo:" + lotNo + " opNo:" + opNo, "", MethodBase.GetCurrentMethod().Name,log);
            ...
            return new SetupLotResult(SetupLotResult.Status.Pass, warningMessage, "LotNo:" + lotNo + " opNo:" + opNo, lotUpdateInfo.Recipe1,MethodBase.GetCurrentMethod().Name,log);
        }
        catch (Exception ex)
        {
    ...
    public StartLotResult StartLot(string lotNo, string mcNo, string opNo,string recipe)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new StartLotResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
         
            //Check package and Lot Pro
            CheckLotApcsProResult proResult = CheckLotApcsPro(lotNo, lotInfo.Package.Name, log);
            if (!proResult.IsPass)
            {
                return new StartLotResult(proResult.Cause, MethodBase.GetCurrentMethod().Name, log);
            }
            ...
            MachineInfo machineInfo = c_ApcsProService.GetMachineInfo(mcNo, log, dateTimeInfo.Datetime);
            if (machineInfo == null)
                return new StartLotResult("ไม่พบ MCNo :" + mcNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);

            LotUpdateInfo lotUpdateInfo = c_ApcsProService.LotStart(lotInfo.Id, machineInfo.Id, userInfo.Id, 0, "", 1, recipe, log,1,-1);
            if (!lotUpdateInfo.IsOk)
                return new StartLotResult(lotUpdateInfo.ErrorNo + ":" + lotUpdateInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);

            return new StartLotResult();
        }
        catch (Exception ex)
        {
            return new StartLotResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    }
    ...
    public UpdateFirstinspectionResult UpdateFirstinspection(string lotNo, string opNo, Judge judge, string mcNo)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new UpdateFirstinspectionResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            //Check package and Lot Pro
            CheckLotApcsProResult proResult = CheckLotApcsPro(lotNo, lotInfo.Package.Name, log);
            if (!proResult.IsPass)
            {
                return new UpdateFirstinspectionResult(proResult.Cause, MethodBase.GetCurrentMethod().Name, log);
            }
            ...
            if (!lotUpdateInfo.IsOk)
                return new UpdateFirstinspectionResult(lotUpdateInfo.ErrorNo + ":" + lotUpdateInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);

            return new UpdateFirstinspectionResult();
        }
        catch (Exception ex)
        {
            return new UpdateFirstinspectionResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    }
    ...
    public UpdateFinalinspectionResult UpdateFinalinspection(string lotNo, string opNo, Judge judge, string mcNo)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new UpdateFinalinspectionResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            //Check package and Lot Pro
            CheckLotApcsProResult proResult = CheckLotApcsPro(lotNo, lotInfo.Package.Name, log);
            if (!proResult.IsPass)
            {
                return new UpdateFinalinspectionResult(proResult.Cause, MethodBase.GetCurrentMethod().Name, log);
            }
            ...
            if (!lotUpdateInfo.IsOk)
                return new UpdateFinalinspectionResult(lotUpdateInfo.ErrorNo + ":" + lotUpdateInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);

            return new UpdateFinalinspectionResult();
        }
        catch (Exception ex)
        {
            return new UpdateFinalinspectionResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    }
    ...
    public EndLotResult EndLot(string lotNo, string mcNo, string opNo, int good, int ng)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new EndLotResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            //Check package and Lot Pro
            CheckLotApcsProResult proResult = CheckLotApcsPro(lotNo, lotInfo.Package.Name, log);
            if (!proResult.IsPass)
            {
                return new EndLotResult(proResult.Cause, MethodBase.GetCurrentMethod().Name, log);
            }
            UserInfo userInfo = c_ApcsProService.GetUserInfo(opNo, log, dateTimeInfo.Datetime, 30);

            MachineInfo machineInfo = c_ApcsProService.GetMachineInfo(mcNo, log, dateTimeInfo.Datetime);
            if (machineInfo == null)
                return new EndLotResult("ไม่พบ MCNo :" + mcNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            ...
            if (!lotUpdateInfo.IsOk)
                return new EndLotResult(lotUpdateInfo.ErrorNo + ":" + lotUpdateInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);

            return new EndLotResult();
        }
        catch (Exception ex)
        {
            return new EndLotResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    }
    ...
    public CancelLotResult CancelLot(string mcNo,string lotNo,string opNo)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new CancelLotResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);

            //Check package and Lot Pro
            CheckLotApcsProResult proResult = CheckLotApcsPro(lotNo, lotInfo.Package.Name, log);
            if (!proResult.IsPass)
            {
                return new CancelLotResult(proResult.Cause, MethodBase.GetCurrentMethod().Name, log);
            }
            ...
            MachineInfo machineInfo =  c_ApcsProService.GetMachineInfo(mcNo, log, dateTimeInfo.Datetime);
            if (machineInfo == null)
                return new CancelLotResult("ไม่พบ MCNo :" + mcNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);

            LotUpdateInfo lotUpdateInfo = c_ApcsProService.LotCancel(lotInfo.Id, machineInfo.Id, userInfo.Id, 1, log);
            if (!lotUpdateInfo.IsOk)
            {
                return new CancelLotResult(lotUpdateInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);
            }
            return new CancelLotResult(); 
        }
        catch (Exception ex)
        {
            return new CancelLotResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    ...
    private ReinputResult AbnormalLotEnd_BackToThe_BeforeProcess(string lotNo, string mcNo, string opNo, int good, int ng, bool holdLot)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new ReinputResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            //Check package and Lot Pro
            CheckLotApcsProResult proResult = CheckLotApcsPro(lotNo, lotInfo.Package.Name, log);
            if (!proResult.IsPass)
            {
                return new ReinputResult(proResult.Cause, MethodBase.GetCurrentMethod().Name, log);
            }
            ...
            MachineInfo machineInfo = c_ApcsProService.GetMachineInfo(mcNo, log, dateTimeInfo.Datetime);
            if (machineInfo == null)
                return new ReinputResult("ไม่พบ MCNo :" + mcNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            ...
            if (!lotUpdateInfo.IsOk)
            {
                return new ReinputResult(lotUpdateInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);
            }
            return new ReinputResult();
        }
        catch (Exception ex)
        {
            return new ReinputResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    }
    #endregion
    ...
    public MachineOnlineStateResult MachineOnlineState(string mcNo, MachineOnline online)
    {
        ...
        {
            return new MachineOnlineStateResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
       // return new MachineOnlineStateResult();
    }
    ...
    public MachineAlarmResult MachineAlarm(string lotNo, string mcNo, string opNo ,string AlarmNo, AlarmState alarm)
    {
        ...
        try
        {
            ...
            LotInfo lotInfo = c_ApcsProService.GetLotInfo(lotNo, log, dateTimeInfo.Datetime);
            if (lotInfo == null)
                return new MachineAlarmResult("ไม่พบ lotNo :" + lotNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);

            MachineInfo machineInfo = c_ApcsProService.GetMachineInfo(mcNo, log, dateTimeInfo.Datetime);
            if (machineInfo == null)
                return new MachineAlarmResult("ไม่พบ MCNo :" + mcNo + " ในระบบ", MethodBase.GetCurrentMethod().Name, log);
            ...
            if (!machineAlarmInfo.IsOk)
                return new MachineAlarmResult(machineAlarmInfo.ErrorNo + ":" + machineAlarmInfo.ErrorMessage, MethodBase.GetCurrentMethod().Name, log);

            return new MachineAlarmResult();
        }
        catch (Exception ex)
        {
            return new MachineAlarmResult(ex.Message.ToString(), MethodBase.GetCurrentMethod().Name, log);
        }
    }
